...
class NCCLWeightBroadcast(WeightBroadcast):
    ...
    def broadcast_weights(self, model: nn.Module, step: int) -> None:
        ...
        # Every trainer rank must wait before DTensor/FSDP collectives begin.
    ...
    def _compute_notified_runs(self) -> list[tuple[int, Path]]:
        """Derive broadcast paths from state replicated across trainer ranks."""
    ...
    def _notify_orchestrator(self, notified_runs: list[tuple[int, Path]]) -> None:
        """Create STABLE markers and clear ready flags on the master rank."""
