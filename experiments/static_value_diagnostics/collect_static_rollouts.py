...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of train rollout groups to collect; cycles over the saved train prompt pool if larger.",
    )
...
async def collect(args: argparse.Namespace) -> None:
    ...
    num_train_groups = args.num_train_groups if args.num_train_groups is not None else len(prompt_pools["train"])
    if num_train_groups < 0:
        raise ValueError(f"num_train_groups must be non-negative, got {num_train_groups}")
    if num_train_groups and not prompt_pools["train"]:
        raise ValueError("num_train_prompts must be positive when num_train_groups is positive")
    max_prompt_id = max(max(ids, default=0) for ids in prompt_pools.values())
    ...
    async def collect_one(split: str, prompt_id: int, group_id: str) -> None:
        nonlocal completed
        if seen_counts.get(group_id, 0) >= args.group_size:
            return
        async with sem:
